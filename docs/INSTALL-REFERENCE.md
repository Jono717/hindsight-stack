> Upstream Hindsight installation documentation, kept verbatim as the reference
> this repo was built from. It covers every deployment option, not just the Docker
> path used here. For the actual setup steps on this machine see
> [../README.md](../README.md).
>
> Source: <https://hindsight.vectorize.io/developer/installation>
...
| **macOS** (Intel / x86\_64) | ✅ | ⚠️ slim only | ✅ | Use `hindsight-all-slim` / `hindsight-api-slim`. The full bundle's local ML models (PyTorch, MLX) publish no Intel-Mac wheels, so `pip install hindsight-all` silently backtracks to a months-old release. Pair the slim bundle with a hosted embeddings/reranker provider or the in-process ONNX backend (`hindsight-api-slim[local-onnx]`). |
| **Windows** (x86\_64) | ✅ | ✅ | ✅ | Fully supported, see [Windows setup](#windows) for external PostgreSQL option |
...
Configure which one to use with `HINDSIGHT_API_VECTOR_EXTENSION`. See [Configuration](https://hindsight.vectorize.io/developer/configuration) for details.
...
You need an LLM API key for fact extraction, entity resolution, and answer generation. See [Models](https://hindsight.vectorize.io/developer/models) for supported providers, model recommendations, and configuration.
...
| **API, Slim image** | 512 MB | 1 GB | No local models. Steady-state RSS is dominated by Python runtime and DB connections. Requires [external embedding and reranker providers](https://hindsight.vectorize.io/developer/configuration#embeddings) (e.g. TEI, OpenAI, Cohere). |
...
- **Control Plane** (Web UI): [http://localhost:9999](http://localhost:9999/)

> [!-secondary] -secondary
> Persisting data: named volume vs. host bind mount
>
...
>
> Set `HINDSIGHT_API_WORKER_ID` to a stable value (e.g., `-e HINDSIGHT_API_WORKER_ID=hindsight-prod`) so the worker keeps the same identity across restarts. This is recommended even for single-container deployments. For diagnosis and recovery commands, see [Admin CLI - Recovering stuck operations](https://hindsight.vectorize.io/developer/admin-cli#recovering-stuck-or-zombie-operations).
...
| **Slim** (`slim`) | ~500 MB | ~500 MB | Use when you already rely on external services for embeddings and reranking (OpenAI, Cohere, TEI). Significantly smaller image, faster deploys. Requires [external providers](https://hindsight.vectorize.io/developer/configuration#embeddings). |

The slim image corresponds to the [`hindsight-api-slim`](#bare-metal-pip) pip package. See [Configuration](https://hindsight.vectorize.io/developer/configuration#embeddings) for external provider options.
...
> [!-success] -success
> Production deployments with non-default local models
>
...
When using `hindsight-api-slim`, you must configure external providers for all model operations. See [Configuration](https://hindsight.vectorize.io/developer/configuration#embeddings) for details.
...
This creates a database in `~/.hindsight/data/` and starts the API on [http://localhost:8888](http://localhost:8888/).
...
| `-a, --api-url` | `HINDSIGHT_CP_DATAPLANE_API_URL` | [http://localhost:8888](http://localhost:8888/) | Hindsight API URL |
...
> tip
>
> You can also use the slim package (`pip install hindsight-api-slim`) if you configure external providers for embeddings and reranking. See [Configuration](https://hindsight.vectorize.io/developer/configuration#embeddings) for details.
...
On Intel (x86\_64) Macs, install `hindsight-all-slim`, see [Supported Platforms](#supported-platforms).
...
See the [Python SDK](https://hindsight.vectorize.io/sdks/python) for the full API reference.

---

## Next Steps

- [Configuration](https://hindsight.vectorize.io/developer/configuration), Environment variables and settings
- [Models](https://hindsight.vectorize.io/developer/models), ML models and providers
- [Monitoring](https://hindsight.vectorize.io/developer/monitoring), Metrics and observability
